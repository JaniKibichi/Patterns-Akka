...
class TrafficLightFSM(target: ActorRef) extends Actor with ActorLogging with FSM[TrafficLightState, Data]{
  import context.dispatcher

  trafficLightState(Green, Yellow, 2)
  trafficLightState(Yellow, Red, 2)
  trafficLightState(Red, Green, 2)
  startWith(Green, Countdown(8))
    ...
    onTransition {
      case Green -> Yellow =>
        target ! ReportChange(Yellow, new Date())

      case Yellow -> Red =>
        target ! ReportChange(Red, new Date())

      case Red -> Green =>
        target ! ReportChange(Green, new Date())
    }

  private def scheduleTick() = context.system.scheduler.scheduleOnce(1 second, self, Tick)

  private def trafficLightState(trafficLightState: TrafficLightState,nextTrafficLightState: TrafficLightState,totalSecondsNextState: Int) = {

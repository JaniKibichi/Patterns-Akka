...
import com.github.janikibichi.learnakka.patterns.SocialMediaAggregator.{Report, StartFetching, StopFetching}
...
class SocialMediaStalker(aggregator: ActorRef, userId: String) extends Actor with ActorLogging{
  ...
  override def postStop() = log.info(s"Stopping. Overall counts for $userId: $counts")

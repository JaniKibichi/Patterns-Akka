...
import com.github.janikibichi.learnakka.patterns.WorkerWorkPulling.{RejectWork, Work, WorkDone}
...
class GeneratorWorkPulling(master: ActorRef) extends Actor with ActorLogging{
  ...
  override def preStart() = context.system.scheduler.schedule(1 second, 1 seconds, self, GenerateWork)
